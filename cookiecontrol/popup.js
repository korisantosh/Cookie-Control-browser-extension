document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderCookies(cookiesList) {
    ...
    cookiesList.forEach((cookie) => {
      ...
      row.innerHTML = `
          <td><div class="cookie-item"><span title="${cookie.name}" class="text-ellipsis">${cookie.name}</span><span class="action-ctas"><svg class="copy-cookie-icon" data-cookie="${cookie.name}" focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="ContentCopyIcon"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"></path></svg></span></div></div></td>
          <td class="cookie-value"><div class="cookie-item"><span title="${cookie.value}" class="text-ellipsis">${cookie.value}</span><span class="action-ctas"><svg title="Copy Cookie" class="copy-cookie-icon" data-cookie="${cookie.value}"  focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="ContentCopyIcon"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"></path></svg><svg class="edit-icon" title="Edit Cookie" data-cookie-name="${cookie.name}" data-cookie-value="${cookie.value}" focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="EditIcon"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34a.9959.9959 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"></path></svg></span></div></td>
        `;
      cookiesBody.appendChild(row);
      // Add event listener for copy button
      // const copyButton = row.querySelector('.copy-cookie-icon');
      // copyButton.addEventListener('click', () => { copyToClipboard(cookie.value); console.log('copy cookie:', cookie.value); });
    });
  }
